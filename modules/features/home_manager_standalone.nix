{
  den.aspects.features.home-manager-standalone = {
    homeManager = { home ? null, lib, pkgs, ... }: {
      ...
      # built with `nixos-rebuild`; only when you bootstrap using manual home manager system to
      # build home manager config for the first time (so it's only for non-nixos systems). i think.)
