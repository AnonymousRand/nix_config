    ...
    homeManager = { user, config, lib, ... }: {
      options = {
        noctaliaTheming = lib.mkOption {
          type = lib.types.submodule {
            options = {
              palette      = lib.mkOption { type = lib.types.attrsOf lib.types.anything; };
              customColors = lib.mkOption { type = lib.types.attrsOf lib.types.anything; };
              templates    = lib.mkOption { type = lib.types.attrsOf lib.types.anything; };
            };
          };
        };
      };
      ...
      config =
        ...
        in
        {
          ...
          # set defaults/base values for options declared above
          noctaliaTheming = {
