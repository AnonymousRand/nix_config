...
# TODO: if using `den.aspects` as the aspect tree root doesn't narrow down to user scope,
# try manually passing tree root as an arg and using `meow` namespace for user
{ den, lib, ... }: {
  # reserve `settings` key so that den does not mistake a `settings` key in an aspect
  # for one of its special aspect keys
  den.reservedKeys = [
    "settings"
  ];

  # (`schema.conf` is applied to all entity kinds)
  den.schema.conf =
    let
      ...
      settingsType =
        let
          # these are keys that are NOT child aspects: structural keys (`includes`, `nixos`, ...),
          # plus your framework's registered class names and quirk/extension keys
          # (adapt these to your own framework)
          inherit (den.lib.aspects.fx.keyClassification) structuralKeysSet;
          classKeys = den.classes or {};
          quirkKeys = den.quirks or {};
          skipKey = k: structuralKeysSet ? ${k} || classKeys ? ${k} || quirkKeys ? ${k};

          # settings declarations may be plain attrsets (`{ foo = mkOption {...}; }`), or module-
          # shaped with explicit `imports`/`config`. give the module keys defaults so attrsets work
          #
          ...
          # build the submodule for one aspect-tree node, mirroring the tree. a node may be BOTH
          # an aspect with `.settings` AND a parent of child aspects that have settings
          # (e.g. `services.bgp' has `localAsn` settings and also parents `services.bgp.cilium-bgp`).
          # merge the node's own settings options with recursion into its settings-bearing children
          nodeModule = node:
            let
              ownSettings =
              ...
              # distinct names so statix (W04) can't rewrite to `inherit (ownSettings) imports`,
              # which would drop the `or` default
        ...
        in
        types.submodule (nodeModule (den.aspects or {})); # reads aspect tree here
    in
    {
      # finally register the `settings` option on entities after collecting all that
      options = {
