...
- generally the reusable features get top-level modules, while non-reusable ones (e.g. user-specific home manager configs) may fall back to lower-level modules like regular nix modules. this helps avoid cluttering the top-level namespace with a bunch of "private" modules. (in the case of `den`, users currently get their own top-level namespace instead of `den.aspects` to store their user-specific configs.)
...
- in den, this is flipped to become `den.aspects.<feature>.<class>`, which means that *every feature now only needs to define ONE module* instead of one per class! (these top-level modules are now called "aspects".) thus it further consolidates the feature-focused approach of dendritic.
...
- but yes if you look past the surface-level unfamiliarity of the new syntax it can be kinda neat and convenient and clear in its intentions (well, besides some of the den-heavy wiring in `modules/core/`). (there are some things that can be harder like defining *aspect*-level custom options instead of nixos module-level ones, but den is still very much a new thing and it will probably evolve more in the future :3)

### `flake.nix` and `flake-file`

- i use [`flake-file`](https://github.com/denful/flake-file) to autogenerate my `flake.nix`. it allows me to put inputs (into `flake-file.inputs`) inside their corresponding feature modules instead of all in `flake.nix`, and `flake-file` aggregates all these inputs into `flake.nix` when `nix run .#write-flake` is run.
- `flake-parts`/den modules still receive the exact same `inputs` argument (which contains all the aggregated `flake-file.inputs`).
